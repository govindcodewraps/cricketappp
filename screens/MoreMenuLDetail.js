...
export default function MoreMLDetail({route}){

    const navigation = useNavigation();
    let zoomableViewRef = useRef();

    const [isLoading, set_isLoading] = React.useState(true);
    const [isModel, set_isModel] = React.useState(false);
    const [refreshing, setRefreshing] = React.useState(false);
    const [DataDetail, Set_DataDetail] = React.useState([]);
    const [ShareUrl, Set_ShareUrl] = React.useState("");

    const FetchData=async ()=>{
        console.log("MoreMenuLDetail.js, Title : ", route.params.title);
        console.log("MoreMenuLDetail.js, url : ", route.params.PageID);
        let Top5;
            Top5 = await DR.Get_MoreMLDetail(route.params.PageID);
            Set_DataDetail(Top5);
            setTimeout(()=> {
                setRefreshing(false);
                set_isLoading(false);
                ...
                <View style={{flexDirection: "column", marginTop: 10, marginBottom: 30,
                  ...
                  <View style={{ backgroundColor: "#000000", width: DEVICEWIDTH * 0.95,
                    ...
                    <Image source={{uri: DataDetail._links["wp:featuredmedia"][0].href}} 
                      ...
                      <Text style={{color: "#000000", fontSize: 13, alignItems: "flex-start", width: DEVICEWIDTH - 50}}>
                          Modify on : {DataDetail.modified}
                      </Text>
                    <View style={{marginLeft: 10}}>
                        <RenderHtml
                            contentWidth={DEVICEWIDTH * 0.6}
                            source={source}
